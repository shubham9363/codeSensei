  ...
  const htmlContent = `
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:480px;margin:0 auto;background:#0a0a0f;color:#e2e2f0;border-radius:20px;overflow:hidden;border:1px solid #2a2a3d;">
      <div style="padding:32px 28px;text-align:center;">
        ...
        <p style="color:#6b6b8a;font-size:13px;margin:0 0 28px;">Debug. Trace. Level Up.</p>
        <p style="font-size:14px;color:#b0b0cc;margin:0 0 20px;">Your password reset code is:</p>
        <div style="background:#1c1c28;border:2px solid #ef4444;border-radius:14px;padding:20px;margin:0 auto 20px;max-width:280px;">
          <div style="font-family:'Courier New',monospace;font-size:36px;font-weight:800;letter-spacing:8px;color:#ef4444;">
            ${otp}
          </div>
        </div>
        <p style="font-size:12px;color:#6b6b8a;margin:0 0 8px;">This code expires in <strong style="color:#f59e0b;">10 minutes</strong>.</p>
        <p style="font-size:11px;color:#4a4a60;margin:0;">If you didn't request a password reset, you can safely ignore this email.</p>
      </div>
      <div style="background:#13131a;padding:16px;text-align:center;border-top:1px solid #2a2a3d;">
        <p style="font-size:10px;color:#4a4a60;margin:0;">© CodeSensei – Learn. Debug. Level Up.</p>
      </div>
    ...
    try {
      await Promise.race([
        transporter.sendMail({
          from: `"CodeSensei" <${smtpUser}>`,
          to: email,
          subject: "CodeSensei – Password Reset",
          html: htmlContent
        }),
        timeout(25000)
      ]);
      ...
      throw new Error(`Gmail SMTP Failed: ${smtpErr.message}`);
    }
  }
...
module.exports = { generateOTP, sendOTP, sendResetOTP };
